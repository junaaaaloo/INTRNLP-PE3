...
def create_table_string(table_data, table_name):
    ...
    for column_name in table_data['columns']:
        ...
        message += "{} {}{}{}".format(
            column_name, column_data['type'], 
            (" PRIMARY KEY" if 'primary_key' in column_data else ""),
            (" AUTOINCREMENT" if 'autoincrement' in column_data else ""))
...
def get_cursor():
    ...
    if (not connection or not has_connection):
